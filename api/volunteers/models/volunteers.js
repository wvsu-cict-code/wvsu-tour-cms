...
module.exports = {
  lifecycles: {
    ...
    async afterUpdate(data) {
      const doc = data._id
      strip_id(data)
      const res = await strapi.firebaseDB.collection('volunteers').doc(JSON.stringify(doc)).update(data);
    },
    async afterDelete(data) {
      const doc = data._id
      strip_id(data)
      const res = await strapi.firebaseDB.collection('volunteers').doc(JSON.stringify(doc)).remove();
    },
  }
};
